...
public enum PartOfSpeechEnum {
    ...
    private static final Logger log = LoggerFactory.getLogger(PartOfSpeechEnum.class);

    private static final Map<String, PartOfSpeechEnum> byTag;
    static {
        byTag = Arrays.stream(PartOfSpeechEnum.values()).collect(Collectors.toMap(item -> item.tag, Function.identity()));
    }

    public static PartOfSpeechEnum getByTag(String tag) {
        PartOfSpeechEnum partOfSpeechEnum = byTag.get(tag);
